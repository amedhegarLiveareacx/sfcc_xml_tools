...
class PriceBookParser {
  ...
  getProductIdPricePair(filename) {
    ...
    if (xmlData) {
      ...
      var priceTables =
        json.pricebooks.pricebook['price-tables']['price-table'];
  ...
  generateCatalogProductPair(priceBookType) {
    console.log(
      '=====================Generating catalog product pair=============='
    );
    ...
    const filename = path.join(
      ...
      'generatedfiles',
      'pricebooks',
      priceBookType + '_pricebookpair.json'
    ); // priceBookType + '_pricebookpair.json';
    ...
    console.log(
      '=====================Finished generating catalog product pair=============='
    );
  }
  ...
  generateProductPricePair(priceBookType) {
    const priceBookPairFileName = path.join(
      __dirname,
      'generatedfiles',
      'pricebooks',
      priceBookType + '_pricebookpair.json'
    ...
    if (filenames) {
      filenames.forEach((filename) => {
        ...
        console.log(
          `===============Finished reading the file:: ${filename} ===================`
        );
      });
    }
  }
  ...
  generatePricebookXml(priceBookType) {
    ...
    const finalPriceBookFileName =
      priceBookType === 'usd_list' ? 'USD-list.xml' : 'USD-sale.xml';
    const outPutFile = path.join(
      __dirname,
      'generatedfiles',
      'pricebooks',
      finalPriceBookFileName
    );
    var priceXml = '';
    ...
    this.writeDataToFile(priceXml, outPutFile);
  }
}
